...
const { valuarPropiedadCompleto, normTipo } = require('./motor_remi_api_lab');  // LAB: para probar flags (LAB_SIZEBAND)
...
const candidatos = cerebro.filter(o =>
    ...
    && !(o.tipo||'').toUpperCase().includes('LOCAL')   // LAB: fuera mixtos (CASA CON LOCAL) — pera con pera, puro casa/depto
...
for (const opi of muestra) {
    ...
    // ── LAB: dos lecturas de MERCADO ──
    // (a) comps del MOTOR (pm2cAvg × m²C) — NO independiente de REMI (circular)
    const pm2cAvg = motorResult.pm2cAvg || 0;
    const valorMercadoComps = (pm2cAvg > 0 && m2C > 0) ? Math.round(pm2cAvg * m2C) : 0;
    // (b) comps del PROPIO PERITO (mediana $/m²C de opi.comparables × m²C) — INDEPENDIENTE del motor
    const pcompsM2 = (opi.comparables||[])
        .map(c => ({ p: parsePesos(c.precio), mc: parsePesos(c.construccion) }))
        .filter(c => c.p > 0 && c.mc > 30)
        .map(c => c.p / c.mc)
        .sort((a,b)=>a-b);
...
// ══════════════════════════════════════════════════════════════════════════════
// LAB FASE 1 — TRIANGULACIÓN perito / REMI / MERCADO (consenso de comps)
// ══════════════════════════════════════════════════════════════════════════════
// Mercado INDEPENDIENTE = comps del propio perito (opi.comparables). Solo OPIs con >=3 comps con m²C.
const tri = recientes.filter(r => r.valorMktPeritoComps > 0 && r.nCompsPeri >= 3 && r.valorPerito > 0 && r.valorMotor > 0);
const gap = (a,b) => Math.abs(a-b)/b*100;
const pct = (n,d) => d ? (100*n/d).toFixed(0)+'%' : '—';
const UMBRAL = 13;

let remiVsPeri=0, remiVsMkt=0, periVsMkt=0;
let subjetivo=0, motorMalo=0, ambosCerca=0;
const detalle = [];
for (const r of tri) {
    const mkt = r.valorMktPeritoComps;               // mercado independiente (comps del perito)
    const gRP = gap(r.valorMotor, r.valorPerito);    // REMI vs perito
    const gRM = gap(r.valorMotor, mkt);              // REMI vs mercado independiente
    const gPM = gap(r.valorPerito, mkt);             // perito vs mercado independiente
    if (gRP <= UMBRAL) remiVsPeri++;
    if (gRM <= UMBRAL) remiVsMkt++;
    if (gPM <= UMBRAL) periVsMkt++;
    // Atribución SOLO cuando REMI difiere del perito (>13%):
    if (gRP > UMBRAL) {
        const v = (gPM > UMBRAL && gRM <= UMBRAL) ? 'perito subjetivo'
                : (gRM > UMBRAL && gPM <= UMBRAL) ? 'MOTOR mal'
                : 'ambos/ninguno';
        if (v==='perito subjetivo') subjetivo++; else if (v==='MOTOR mal') motorMalo++; else ambosCerca++;
        detalle.push({ folio:r.folio, peri:Math.round(r.valorPerito/1000)+'k', remi:Math.round(r.valorMotor/1000)+'k', mktPeri:Math.round(mkt/1000)+'k',
            'REMIvsPeri':gRP.toFixed(0)+'%', 'peri-vs-mkt':gPM.toFixed(0)+'%', 'REMI-vs-mkt':gRM.toFixed(0)+'%', veredicto:v });
    }
}
console.log('\n══════════════════════════════════════════════════════════════');
console.log(`LAB FASE 1 — TRIANGULACIÓN con MERCADO INDEPENDIENTE (comps del perito)`);
console.log(`(${tri.length} OPIs 2024-2026 con >=3 comps con m²C, umbral ±${UMBRAL}%)`);
console.log('══════════════════════════════════════════════════════════════');
console.log(`  REMI dentro de ±${UMBRAL}% del PERITO         : ${remiVsPeri}/${tri.length} (${pct(remiVsPeri,tri.length)})`);
console.log(`  REMI dentro de ±${UMBRAL}% del MERCADO indep. : ${remiVsMkt}/${tri.length} (${pct(remiVsMkt,tri.length)})   ← fidelidad del MOTOR al mercado`);
console.log(`  PERITO dentro de ±${UMBRAL}% del MERCADO indep.: ${periVsMkt}/${tri.length} (${pct(periVsMkt,tri.length)})   ← fidelidad del PERITO al mercado`);
const fallaRP = tri.length - remiVsPeri;
console.log(`\n  De los ${fallaRP} donde REMI se aleja del perito (>±${UMBRAL}%), ¿a quién le da la razón el mercado?`);
console.log(`    • PERITO subjetivo (mercado respalda a REMI): ${subjetivo} (${pct(subjetivo,fallaRP)})`);
console.log(`    • MOTOR mal (mercado respalda al perito)     : ${motorMalo} (${pct(motorMalo,fallaRP)})`);
console.log(`    • ambos/ninguno cerca del mercado           : ${ambosCerca} (${pct(ambosCerca,fallaRP)})`);
console.log('\n  Detalle de los desacuerdos REMI–perito:');
