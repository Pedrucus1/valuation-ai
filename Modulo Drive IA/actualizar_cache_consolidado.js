...
async function main() {
    ...
    const raw = rows
        ...
        .map(r => ({
            precio:  Math.round(cleanNum(r[COL.precio])),
            m2c:  Math.round(cleanNum(r[COL.m2c])),
            m2t:  Math.round(cleanNum(r[COL.m2t])) || 0,
            tipo: (r[COL.tipo_prop] || '').toLowerCase(),
            colonia: (r[COL.colonia]   || '').toLowerCase(),
            muni: (r[COL.municipio] || '').toLowerCase(),
            recamaras: cleanNum(r[COL.rec])  || null,
            banos: cleanNum(r[COL.ban])  || null,
            estac: cleanNum(r[COL.estac]) || null,
            fecha: (r[COL.fecha_scrap] || '').toString().slice(0, 10) || null,
    ...
    let corregidos = 0;
    for (const d of raw) {
        if (TIPOS_TERRENO.some(tt => d.tipo.includes(tt)) && d.m2t === 0 && d.m2c > 0) {
            d.m2t = d.m2c;
            d.m2c = 0;
            corregidos++;
        }
    ...
    const compacto = raw.filter(d => {
        const col = normStr(d.colonia);
        // Agrupar precio en bloques de 1% para tolerancia de redondeo
        const pKey = Math.round(d.precio / (Math.max(d.precio, 1) * 0.01));
        const area = d.m2c > 0 ? d.m2c : d.m2t; // usar m2t cuando m2c=0 (terrenos corregidos)
    ...
    const conEspacios = compacto.filter(d => d.recamaras || d.banos).length;
    const conFecha = compacto.filter(d => d.fecha).length;
