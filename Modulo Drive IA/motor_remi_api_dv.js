...
const INDEX_PATH         = path.join(__dirname, 'cache_index_dv.json');  // #121 experimento
...
const { coloniasCercanas, coordsDeColonia, haversine } = require('./_geo/proximidad.cjs');  // #121 geo
...
function valuarPropiedad(prop) {
    ...
    const scored = candidatos.map(d => {
        ...
        return { precio: d.precio, m2_const: d.m2c, score: s, an: d.anio || null,
                 m2t: d.m2t || 0, rec: d.recamaras || 0, ban: d.banos || 0, estac: d.estac || 0, col: dc };  // #121 DV
    ...
    // ── #121 DV: ponderación por disimilitud (Mahalanobis-lite estandarizado) + N chico ──
    // Reemplaza el promedio PLANO por suma ponderada inversa a la distancia del comp al sujeto
    // sobre {m²C, m²T, recámaras, baños, año} estandarizados. Conserva ajustes tamaño/edad/conserv.
    // Variable faltante (comp o sujeto = 0) no penaliza: se omite del cálculo de esa distancia.
    const DV_N   = parseInt(process.env.DV_N   || '0');     // 0 = usar todos; >0 = top-N por DV
    const DV_EPS = parseFloat(process.env.DV_EPS || '0.5'); // suaviza peso cuando dv≈0
    const _curY  = new Date().getFullYear();
    const _subj  = { m2c: m2C, m2t: m2T || 0, rec: prop.recamaras || 0, ban: prop.banos || 0,
                     estac: prop.estacionamientos || 0, anio: edad > 0 ? _curY - edad : 0 };
    const _cv = (c, v) => v==='m2c' ? c.m2_const : v==='m2t' ? (c.m2t||0) : v==='rec' ? (c.rec||0)
                        : v==='ban' ? (c.ban||0) : v==='estac' ? (c.estac||0) : (c.an||0);
    const _VARS = ['m2c','m2t','rec','ban','anio'];
    if (process.env.DV_ESTAC === '1') _VARS.push('estac');  // #121: toggle para A/B del estacionamiento
    const _std = {};
    ...
    // #121 geo: penalización por distancia física (Krause: DV = dist_atributos + dist_geo/k). DV_K=0 → apagado.
    const DV_K     = parseFloat(process.env.DV_K || '0');   // km por punto de penalización (mayor k = menos peso geo)
    const _subjGeo = DV_K > 0 ? coordsDeColonia(colNorm, muniNorm) : null;
    const _geoKm = (c) => {
        if (!_subjGeo || !c.col) return 0;
        const cg = coordsDeColonia(c.col, muniNorm);
        if (!cg) return 0;
        return haversine(_subjGeo.lat, _subjGeo.lon, cg.lat, cg.lon);
    };
    const _DV = (c) => {
        let s=0, k=0;
        for (const v of _VARS) { const cv=_cv(c,v), sv=_subj[v];
            if (cv>0 && sv>0) { const d=(cv-sv)/_std[v]; s+=d*d; k++; } }
        const attr = k ? Math.sqrt(s/k) : 1;   // RMS normalizado por #vars usadas
        return DV_K > 0 ? attr + _geoKm(c)/DV_K : attr;
    };
    let _scoredDV = compsFilt.map(c => ({
        adj: (c.precio/c.m2_const) * Math.pow(c.m2_const/m2C, 1/6) * factorEdad * factorConserv,
        dv:  _DV(c)
    })).sort((a,b) => a.dv - b.dv);
    if (DV_N > 0) _scoredDV = _scoredDV.slice(0, DV_N);
    let _w=0, _vv=0;
    _scoredDV.forEach(({adj,dv}) => { const w = 1/(dv+DV_EPS); _vv += adj*w; _w += w; });
    const _flat = compsFilt.reduce((a,c)=>a+(c.precio/c.m2_const)*Math.pow(c.m2_const/m2C,1/6)*factorEdad*factorConserv,0)/compsFilt.length;
    let pm2cAvg = _w > 0 ? _vv/_w : _flat;
    // Experimento: DV solo en pools heterogéneos; exacta (comps misma colonia) → promedio plano
    if (process.env.DV_SKIP_EXACTA === '1' && poolTipo === 'exacta') pm2cAvg = _flat;
    pm2cAvg *= parseFloat(process.env.DV_CAL || '1');  // #121: recentrado de sesgo (experimento)
