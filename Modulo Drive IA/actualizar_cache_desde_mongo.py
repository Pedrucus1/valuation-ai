...
def main():
    ...
    for r in col.find(filtro, proj):
        ...
        d = {
            "precio": round(precio),
            "m2c": round(m2c),
            "m2t": round(clean_num(r.get("m2_terreno"))) or 0,
            "tipo": str(r.get("tipo_propiedad", "")).lower(),
            "colonia": str(r.get("colonia", "")).lower(),
            "muni": str(r.get("municipio", "")).lower(),
            "recamaras": clean_num(r.get("recamaras")) or None,
            "banos": clean_num(r.get("banos")) or None,
            "estac": clean_num(r.get("estacionamientos")) or None,
            "fecha": fs,
        }
        if an is not None:
            d["anio"] = an
    ...
    corregidos = 0
    for d in raw:
        if any(tt in d["tipo"] for tt in TIPOS_TERRENO) and d["m2t"] == 0 and d["m2c"] > 0:
            d["m2t"] = d["m2c"]
            d["m2c"] = 0
            corregidos += 1
    if corregidos:
    ...
    compacto = []
    for d in raw:
        coln = norm_str(d["colonia"])
        p_key = round(d["precio"] / (max(d["precio"], 1) * 0.01))  # ≡100 (mismo comportamiento que JS)
    ...
    con_an = sum(1 for d in compacto if d.get("anio") is not None)
    meta = {
