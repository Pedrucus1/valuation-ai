...
const MAESTRO_PATH       = process.env.LAB_MAESTRO_PATH || path.join(__dirname, 'colonias_maestro.json');
...
function valuarPropiedad(prop) {
    ...
    const scored = candidatos.map(d => {
        ...
        if (process.env.LAB_SOFT_NSE === '1' && nseSubjeto) {
            ...
            // ponytail: dc mezcla comps de varios municipios en este pool general, sin campo
            // municipio por registro — usa llave plana (mismo comportamiento que antes del fix).
            // Solo afecta el peso experimental LAB_SOFT_NSE_W (off por defecto).
