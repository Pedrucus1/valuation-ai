...
**★ LECCIÓN CLAVE (edad×clase, no aplicar edad plana):** la misma depreciación que arregla A **EMPEORA Seattle −30→−50**. Seattle es viejo+grande pero en colonia PREMIUM donde el perito ya lo valúa a **0.38× los comps** de su colonia; depreciar a un sujeto que ya está muy por debajo de su zona lo hunde. **Regla que emerge: viejo+popular (perito ~0.55× comps) NECESITA depreciación; viejo+premium-con-sujeto-ya-barato (perito ~0.38× comps) NO.** El discriminante no es la edad sola sino **cuán lejos está el perito de la mediana de comps de su colonia**. Cluster B (grandes) es mecánica de TAMAÑO (`^(1/6)` + DV elige comps grandes baratos), aparte. **Config cluster-A queda medida pero NO commiteada (gana +1/errAbs−0.6 pero hunde Seattle; conviene un discriminante clase antes de portar).**

**✅ RESUELTO Y PORTADO A PROD (discriminante de clase):** el discriminante que emergió (posición del sujeto vs comps de su colonia) se codeó como `_segRatio = mediana(comps seleccionados) / medianaPm2c(colonia)`. En exacta, deprecia el exceso de edad **solo si `_segRatio ≥ 0.75`** (sujeto al nivel de su colonia → el exceso es edad, El Batán 0.93×); si `<0.75` (sujeto ya en sub-segmento barato, Seattle 0.47×) → factor 1.0, no se toca. Constantes: K=0.010, GAP=25 (sobre anclaEdad), FLOOR=0.55. Robusto a SEGMIN 0.70-0.80. **Portado a `motor_remi_api.js` (L999, sin flag). Validador PROD (validar_40_opis.js, 137 OPIs): ±10 64.1→63.1 (−1), ±15 72.8→74.8 (+2.0), ±20 81.6→83.5 (+1.9), errAbs 11.5→10.8 (−0.7).** Seattle protegido (−29.9, no se hunde). Backup `motor_remi_api.PRE_EDADEXACTA_*.bak.js`. Flag `LAB_EDAD_EXACTA`(+`_K/_GAP/_FLOOR/_SEGMIN`) queda en lab. **Cluster B (grandes: DV elige comps grandes-baratos) sigue pendiente — mecánica de tamaño, aparte.** Deploy Railway pendiente.

**Cluster B investigado (NO fix limpio, documentado):** triangulación confirma que el motor SUBVALÚA casas grandes vs MERCADO (comps del propio perito): Educadores mercado $4,379k/motor $2,742k (−37%), Santa María −37%, Las Bóvedas mercado $6,326k/motor $4,109k (−35%). NO es perito optimista — el mercado respalda valores más altos. **PERO el fix no es limpio:** (a) el DV elige los comps grandes por cercanía de tamaño, que en el caché son los más BARATOS ($15-19k en Las Bóvedas), rechazando los grandes caros ($24-25k, más lejos en tamaño); (b) flag `LAB_DV_M2CW` (bajar peso m2c en DV) → **cero efecto** porque el ajuste `^(1/6)` descuenta los comps chicos que entrarían, cancelando el cambio + ligera regresión; (c) los comps grandes del caché tienen tamaño ~igual al sujeto → size-power ~1, no es la palanca. **Raíz probable: rango real de mercado ($15-25k para grandes en la colonia) y el perito usó el extremo alto; o falta de comps grandes de gama alta en el caché.** Ambiguo (rango de mercado vs sesgo perito). NO forzar sin más datos o riesgo al DV validado. Flag LAB_DV_M2CW queda en lab (default 1.0 = prod). **Próximo si se retoma: revisar si el caché tiene cobertura de casas grandes de gama alta por colonia (dato), no fórmula.**

## 🏁 TECHO DE FÓRMULA ALCANZADO 09-Jul — lo que queda es DATO (re-análisis post cluster-A)
Tras portar cluster-A (edad-exacta), re-corrí la triangulación de mercado sobre los 32 desacuerdos: **14 "MOTOR mal" (mercado respalda perito), 17 "ambos/ninguno" (ni motor ni perito cerca del mercado = ruido/dato), 1 "perito subjetivo".** Los 14 reales = **9 casas + 5 deptos** (los "-OF" son numeración de folio, NO oficina — corregido); **11 de 14 SUBvalúan.** Verificado que el fix de edad NO los causó (idénticos con/sin `LAB_EDAD_EXACTA`, salvo Insurgentes −30→−34 que ya fallaba). **Direcciones mezcladas (11 bajo, 3 alto) → NO hay patrón de fórmula único que atacar; son comps/dato (caché de esas colonias más barato que el mercado).** Coincide con el análisis previo: techo de fórmula pura ~81-82%, mejora viene de DATOS. **Estamos en ±20 81.2 (lab) / 83.5 (prod). Decisión: NO forzar más fórmula (riesgo regresión > ganancia). El lever es DATO — servido ahora por la búsqueda web Tavily en vivo + flywheel (acumularComps guarda comps web en Mongo).** OJO: el veredicto "MOTOR mal" usa comps del perito como mercado, que pueden estar sesgados alto (ver cluster B) → parte de esos 14 quizá ni son errores reales. NO re-investigar 1×1 (ver [[feedback_motor_no_cazar_atipicos]]).

**★★ CLUSTER B NO ES BUG (validado con Google directo, 09-jul):** usé búsqueda web propia (sin Serper) para ver el mercado REAL de las casas grandes. Las Bóvedas: listings reales 223m²@$15,470, 130m²@$21,154 → mercado $15-21k = **coincide con el MOTOR ($17k), NO con el perito ($21-25k)**. Educadores: 190m²@$11,000, sujeto 251m² → mercado ~$11-13k = **coincide con MOTOR ($10.9k), NO perito ($15.4k)**. **El veredicto "MOTOR mal" de la triangulación estaba SESGADO: usaba los comps del propio perito como "mercado", pero esos comps son más caros que los listings reales.** Por el principio del usuario (benchmark=MERCADO, no perito), el motor está más cerca del mercado en casas grandes; el perito las valúa alto. **NO "arreglar" cluster B para igualar al perito — empujaría el motor lejos del mercado.** (Alta varianza en grandes; el sujeto puede ser mejor que los baratos, pero en comps puros el motor es defendible.) LAB_DV_M2CW / LAB_BIG_WEB quedan como experimentos pero cluster B deja de ser prioridad de fix.

**⚠️ HALLAZGO OPERATIVO (09-jul): SERPER SIN CRÉDITOS — búsqueda web del motor MUERTA.** Al probar path(2) de cluster B (flag `LAB_BIG_WEB`: relajar la guarda del 10% en el complemento web para propiedades grandes), el web devolvía 0 comps. Causa: `serper.dev` responde **HTTP 400 "Not enough credits"**. Impacto: TODO el complemento/fallback web (buscarCompsConWeb → Serper+DeepSeek) está inactivo → el motor corre **solo con caché** (afecta todas las colonias de caché delgado, no solo cluster B; en offline no se nota, en prod sí). **Acción: recargar créditos Serper (o revisar billing de la cuenta).** El flag `LAB_BIG_WEB` (relajar guarda 10%→60% para m²C≥180) queda gateado en lab — es el lever correcto de cluster B PERO no se puede validar hasta que Serper tenga créditos. Gemini sigue como fallback de último recurso (buscarCompsGemini) pero solo se dispara con nComps<3. Ver [[credentials_registry]]. — DIAGNÓSTICO REVISADO: NO es sistémico, es DATO
...
**RECONCILIACIÓN sobre los 103 puntuados exactos (13-jul, `diff103.py` + dump gateado por env `DUMP` en `validar_40_opis.js`, ya revertido):** reproduce el headline (7-jul mediana +7.2 / 13-jul −7.4). Cambio del diff(motor/perito) por-OPI: **mediana +0.0pp, media +0.2pp, 75% de OPIs se mueven ≤3pp** (5% bajan >10pp, 6% suben >10pp — mixto; varios movers MEJORAN, p.ej. OPI-25-3-21-AV +25→+2.4 al ganar 1 comp).

**⚠️ HALLAZGO CLAVE — la métrica "mediana" del validador es ENGAÑOSA (no es mediana de diffs):** `validar_40_opis.js:249-250` ordena por |diff| y devuelve el diff **con signo del ÚNICO OPI en el rango 52/103**. El salto +7.2→−7.4 es **otro OPI cayendo en esa posición de rango, NO un corrimiento de la distribución** — prácticamente ruido. NO leerlo como "el motor valúa 14% más bajo". **Las métricas robustas: pass-rates (±10 −2.9pp, ±15 −3.0pp, ±20 plano) + errAbs (+0.9pp). La regresión del rebuild es CHICA (~3pp), no el colapso que sugería el −14.6pp de "mediana".**

**Conclusión final (medida y reconciliada):** el 7-jul NO está "stale" (mercado ~igual pool −2.9%, 75% de OPIs quietos). El rebuild regresa ~3pp por una minoría de celdas con comps cambiados (mixto). **Acción: mantener 7-jul, NO reconstruir** (marginalmente mejor, ~3pp). Pero el rebuild NO es catastrófico — si algún día conviene datos frescos, el costo real es ~3pp de pass-rate, no −14pp. Nota para futuro: **reportar pass-rates + errAbs, ignorar la "mediana" del validador o arreglarla a `st.median(diffs)` real.**
...
**Acción:** revertido builder + `mongo_comparables.py` (caché restaurado). Remate seguirá fuera vía `activo=false`/dedup interno. El marcado `duplicado`/`es_remate` en Mongo se conserva (lo usa el verificador). Palanca real = datos/segmentación.

**EXPLORACIÓN dedup del motor (13-jul, MEDIDA):** hay DOS dedups ya: (1) builder `colonia+m2c` → 1 por (colonia,tamaño), colapsa propiedades distintas, quita 19,173; (2) `build_cache_index.js:68` `precio(±)+m2c` = mismo anuncio. **Opción A probada:** quitar la dedup cruda del builder (dejar solo la de precio+m2c). Pool 21k→**39k comps**. Resultado: **±10 63.1→62.1, ±15 74.8→71.8, ±20 83.5→84.5, errAbs 10.8→11.5, mediana +7.2→−7.4.** Mixto/net PEOR (mejora cola ±20 pero empeora núcleo ±10/±15 y errAbs, y voltea el bias a subvaluación). **CONCLUSIÓN: la dedup cruda colonia+m2c, aunque colapsa comps reales, está en un óptimo local — más comps empujan la mediana abajo (asking prices sesgan alto, menos comps calibra mejor vs perito). NO tocar el dedup del builder sin re-medir. El dedup del motor NO es palanca; la palanca es DATOS/segmentación.**
...
**❌ RE-CONFIRMADO 23-Jul (caso Cuarzo) + Ross-Heidecke MEDIDO Y DESCARTADO como curva alterna.** Re-corrida `LAB_EDADSEG=1 K=0.5` (bucket 3-escalones) con datos de hoy (81 OPIs válidas, tras enrichers scoped del 23-jul): reproduce el hallazgo — ±10 60.5%→60.5% (flat), **±15 74.1%→76.5% (+2.4pp)**, ±20 81.5%→81.5% (flat), errAbs 11.7%→11.5%. Mediana se voltea de signo (−7.8%→+8.0%, misma magnitud) — no es regresión pero vale la pena vigilarlo si se activa en prod.

Se agregó `LAB_EDADSEG_MODE=rh` (curva continua Ross-Heidecke `max(0.20,1−0.5(x+x²))`, vidaUtil=70, mismo mecanismo de blend por K) como variante para competir contra el bucket, motivado por un cálculo manual en el caso Cuarzo (Bosques de la Victoria) que dio un número plausible. Medido contra el mismo validador:
- RH K=1.0: ±10 **55.6%** (−4.9pp, REGRESIÓN), ±15 75.3% (+1.2pp, menos que el bucket), ±20 **80.2%** (−1.3pp, REGRESIÓN), errAbs 12.1% (peor).
- RH K=0.5: idéntico al baseline en las 3 métricas de cobertura (60.5/74.1/81.5, errAbs 11.7) — no-op, solo voltea la mediana igual que el bucket.

**Conclusión: Ross-Heidecke NO le gana al bucket de 3 escalones en ningún K probado — descartado como curva. El ganador sigue siendo `LAB_EDADSEG=1 K=0.5` (bucket).** El resultado de Cuarzo con RH cayó en rango razonable por la banda de edad filtrada a mano (25-65 años), no por la curva en sí. Código de la variante RH queda en `motor_remi_api_lab.js` detrás del flag, sin activar. NO re-investigar RH salvo con datos/mecanismo distintos.

## ⚠️ 23-Jul — `calculate_valuation` / `mongo_comparables.py` (pipeline REPORTE, distinto de `motor_remi_api.js`) — Cuarzo real sobrevalúa 42%, causa diagnosticada, 2 intentos de fix MEDIDOS Y DESCARTADOS

**Contexto:** el pipeline que genera los reportes reales (`backend/server.py: generate_comparables` → `mongo_comparables.py: search_comparables_from_mongo` → `calculate_valuation`) es **código Python completamente separado** de `motor_remi_api.js`/`motor_remi_api_lab.js` (el que se valida con los 103 OPIs). No comparten lógica ni se benefician de las palancas ya probadas ahí (LAB_EDADSEG, DV, etc.) — cualquier mejora al motor JS NO se propaga a los reportes reales.

**Caso real:** avalúo Cuarzo (Calle Cuarzo 2380, Bosques de la Victoria, depto 76m²C, 45 años, `val_43a05a7a5511`, 21-jul) dio `estimated_value=$3,931,206`. Mi estimado manual segmentando nuevo/usado + banda de edad daba ~$2.29M. **Ixtepete y La Calma (mismo pipeline) dieron valores cercanos a un cálculo manual equivalente (−3% y −10%)** — el problema NO es generalizado, es específico de BV.

**Causa raíz identificada:** `mongo_comparables.py::_similarity_score()` rankea comparables SOLO por proximidad de m²construcción (60%) y precio (40%, inactivo si no hay `precio_referencia` — que es el caso normal en una valuación nueva, `property_data.value=None`). **La edad nunca participa en el ranking de similitud.** El pool "zona exacta" de BV tiene mediana **$64,511/m²C** en crudo (dominado por PINCALI obra nueva 2026, ya documentado desde el inicio del caso Cuarzo) — el sanity-check de rango (`ref_ppm×0.35` a `×2.5`) es demasiado ancho para filtrar esto.

**Fix #1 descartado — homologación por edad en `calculate_valuation` (bucket EDADSEG, K=0.5, mismo mecanismo que ganó en el motor JS):** recalculados los 32 avalúos guardados (19 con datos completos). **Resultado: mediana +0.0%, máximo −2.3%, NO-OP.** Causa: los comparables GUARDADOS en cada avalúo casi no traen `age` (0-30 de 15-50 comps por caso, mayoría en 0). Sin edad en los comps, no hay nada que homologar. Confirma que el blocker de cobertura de edad que ya se documentó en el motor JS también aplica aquí, posiblemente peor.

**Fix #2 descartado — agregar edad a `_similarity_score()` (mismo peso relativo que el término de m²):** verificado directo con el pool real de Mongo (colonia exacta BV + 16 similares, 707 candidatos, mismo sanity-check x0.35-x2.5 que usa la función real). Top-50 SIN edad en el score: mediana $51,067/m²C. Top-50 CON edad agregada: mediana **$55,000/m²C — SUBIÓ, no bajó**, y el n con edad conocida cayó de 30 a 8 (la ponderación por edad, con tan pocos comps que la tienen, es ruido, no señal, y en este caso empujó hacia arriba en vez de hacia abajo).

**Conclusión: ningún ajuste SUAVE de ponderación (ni homologación tipo Ross-Heidecke/EDADSEG, ni agregar edad al similarity score) arregla Cuarzo.** Lo único que sí funcionó fue mi cálculo manual, que usa un **filtro DURO** (excluir por completo obra nueva edad≤3, no solo bajarle peso) antes de calcular la mediana. Esto reconfirma — con evidencia nueva y directa sobre el pipeline de reportes — que el bloqueador real es el split categórico **NSE nuevo/usado** (`project_propvalu_nse_nuevo_usado`), no una recalibración de pesos. **NO reintentar ajustes de ponderación suave en `mongo_comparables.py`/`calculate_valuation` sin antes resolver el split nuevo/usado.**

**Fix aplicado y SÍ desplegado (bajo riesgo, no requiere medición):** `land_area` en `ValuationForm.jsx` ya no defaultea a `construction_area` cuando se omite — ahora defaultea a `0`. El fallback viejo inflaba el método físico (20% del peso) para CUALQUIER departamento sin importar el piso (asumía terreno propio = área construida completa, absurdo para pisos altos). El campo sigue disponible y opcional para el caso real de planta baja con terreno excedente — el usuario simplemente lo llena si aplica, ya no se adivina.
...
**LECCIÓN:** el lever NO es "cobertura de año" — es **inventario de deptos USADOS**, que es genuinamente escaso (6% de lo recuperable). El parche `gap6` sigue siendo lo mejor (funciona justo depreciando el sujeto viejo contra el pool sesgado-nuevo). Artefactos LAB borrados; para retomar, reconstruir el diff (relajar query `m2c>0` + mapear t->c depto en el builder) **y filtrar obra nueva (año≥2024) del pool depto ANTES** de graduar. NO graduar el t->c sin ese filtro.
