...
### ✅ DERIVA DE DATOS 7→13 jul — INVESTIGADA Y RECONCILIADA (13-jul, offline). La regresión es CHICA (~3pp), no −14pp.
**Resultado:** el rebuild NO es el colapso que parecía. Reconciliado sobre los 103 puntuados: pass-rates ±10 63.1→60.2, ±15 74.8→71.8, **±20 plano**, errAbs 10.8→11.7 → regresión real **~3pp**. El mercado NO bajó (pool $/m²c −2.9%, 75% de OPIs se mueven ≤3pp, movers mixtos, varios MEJORAN). **La "mediana +7.2→−7.4" que asustaba es un MÉTRICO ENGAÑOSO** del validador (`validar_40_opis.js:249` = signo del ÚNICO OPI en rango 52/103, no mediana de diffs → ruido). Detalle + `diff103.py`/`diff_drift.py` → `MOTOR_ANTECEDENTES.md`. Archivos: `cache_*.BASELINE_07JUL*` + `cache_consolidado.REBUILD_13JUL.json`.
**Decisión firme:** mantener caché 7-jul (marginalmente mejor ~3pp), **NO reconstruir prod ahora.** Pero el rebuild NO es catastrófico: si algún día conviene datos frescos, cuesta ~3pp, no −14pp. **Ojo métrico:** reportar pass-rates + errAbs; la "mediana" del validador es ruido (o arreglarla a `st.median` real). Palanca real = datos/segmentación, no rebuild.
- **✅ Deploy menor HECHO (13-jul):** `edades.py` (excluir dup/remate de la lista del verificador) + revert `mongo_comparables.py` desplegados a Railway prod (deployment `f25b7269`, healthy). Backend live ahora filtra dup/remate de la lista por zona. Permiso `railway up/status/logs` NO se agregó a settings (el candado de deploy-a-prod requiere acción humana; se corrió vía `!railway up`).
