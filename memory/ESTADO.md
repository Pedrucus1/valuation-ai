...
- **H ✅ CERRADO** (comps + enricher + post-scrape automatizados; PINCALI data-capped):
  1. **Motor — NO excluir dup/remate de comps ❌ (PROBADO NEGATIVO 13-jul, REVERTIDO).** El motor usa el CACHÉ (`cache_index.json ← cache_consolidado.json ← builder`), NO `search_comparables_from_mongo` (esa función es camino secundario e INERTE por bug de casing `Casa`/`casa`). Filtrar `duplicado`+`es_remate` en el builder = **±10 −2.9, ±15 −3.0, ±20 flat, errAbs +0.9** (pool 21k→18.2k) → revertido (caché restaurado, filtro fuera del builder Y de `mongo_comparables.py`). Detalle en MOTOR_ANTECEDENTES. **Los comps extra ayudan la mediana.** El **verificador** SÍ los excluye de la lista a verificar (esfuerzo humano, no motor — `edades.py`, falta deploy).
