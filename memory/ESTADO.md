...
- **H ⏳ PENDIENTE:**
  ...
  3. **Por-scrape ✅ AUTOMATIZADO:** (a) **remate al vuelo** en `scheduler._guardar_en_mongo` (marca `es_remate` por-doc al guardar, regex afinado). (b) **`dedup_estricto.py`** nuevo (standalone idempotente, campos canónicos, misma lógica) se corre **post-enricher** (hook en `scheduler.run()` tras el auto-enricher, bloqueante). Dry-run valida 2,113 grupos/3,734 dups. Para full-run (portal=None) el enricher lo maneja el watchdog → conviene además **schedular `dedup_estricto.py` diario** (aún no hecho). Conviven 2 convenciones: `es_duplicado_secundario` (viejo, fusionar_duplicados.py) vs `duplicado`/`es_canonico`/`canonico_id` (nuevo, estricto).
