...
| 120 | 🔄 | **Metodología homologación CUS del perito — EL CLAVO ENCONTRADO (29-jun)** — Validación 40-OPI: gateada a **CUS<0.5** arregla el segmento terreno-pesado donde el motor se rompe: subconjunto CUS<0.5 (n=14) motor **21.5%**→metodología **8.1%** (OPI-26-5-16 −52%→+2%). Híbrido global motor 14.4%→13.5%, **daña 1/arregla 7**. Metodología SOLA no gana (14.8 vs 14.4) — solo gateada. **Receta:** `pu=precio/m²C+pm2T·(1/cus_s−1/cus_c)` × `(cus_s/cus_c)^⅙·(m²T_s/m²T_c)^⅙` × CAL_conserv (BUENO .95/REGULAR .89/REMOD .90) [corrige +10% sobrevaluo, pieza faltante]; pm2T=p30 comps terreno; gate CUS<0.5 REEMPLAZO. Réplica pura=0.0% en 6 OPIs. **Integración pendiente:** motor descarta m²T de comps (motor_remi_api.js L855/928)+sin edad/comp → retener m²T+pm2T por zona+gate. Scripts: `validar_40.py`,`calibrar_perito.py`,`decompose_factores.py`,`replicar_perito.py`. Detalle: memoria `project_motor_homologacion_cus`. |
