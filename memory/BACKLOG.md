...
| 135b | ⏳ | **CRÍTICO — correcciones del perito "sticky" (no se pisan al re-scrapear)** — id_unico = md5(url) → NO duplica, pero el scraper/enricher escriben `colonia` en cada corrida y **NO respetan `colonia_fuente`** (grep: cero referencias). Sin guardia, cada mes se borra la corrección humana. FIX: en `scheduler._guardar_en_mongo` y en el write del enricher, NO tocar `colonia`/`conjunto`/`anio_construccion` cuando `colonia_fuente`/`edad_fuente` ∈ {perito_correccion, usuario_correccion, perito_crowdsource}. Guardia de 1 línea en el `$set`, pero indispensable antes de la próxima corrida mensual. |
