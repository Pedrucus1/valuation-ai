...
> **Última actualización:** 02 Jun 2026 — Seguridad (#64): recuperado del corte de sesión y verificado el cierre de IDOR en avalúos + escalada de privilegios en upgrade-role (commit 4e15e10). #66.2 y #66.6 ya estaban hechos (commit c9bdaf4). Nuevo doc maestro `SEGURIDAD_ARQUITECTURA_ANTECEDENTES.md`. Pendientes S1–S6 catalogados ahí.
>
...
| 91 | 🔄 | **Opción 3: capturar edad en scraper — RESUELTO la fuente (01-Jun PM).** propiedades.com (73k, el más grande) ya entrega `año_construccion` vía enricher Node (#75). `actualizar_cache_consolidado.js` mapea col 14 → `an`; `an` cargado en comps. **Auditoría de edad por portal (02-Jun, `_verif_edad.py`):** PROPIEDADES_COM = única fuente estructurada confiable (`amenities.age` numérico + 'Nuevo'); CYT `features.age=0` es default "sin dato" (no obra nueva); INMUEBLES24/PINCALI solo mencionan edad en texto libre (requeriría IA, no regex); VIVANUNCIOS/MITULA sin dato. **Fixes 02-Jun (commit 3581448):** CYT age=0 ya no inventa anio=2026 (afectaba 21,887 props); propiedades.com 'Nuevo'→anio actual (antes se perdía, frecuente en muestra). **Pendiente:** correr enricher a escala + refrescar cache + medir. Oportunidad futura: extraer edad del texto libre de INMUEBLES24/PINCALI vía IA. |
