...
| 135 | ⏳ | **Enricher PINCALI — colonia autoritativa (de-riesgado, listo para implementar)** — fetch `/inmueble/` (español, 200) + parsear **BreadcrumbList JSON-LD** (último nodo = colonia). `enricher.py` ya lo tiene (scrapes nuevos OK); falta backfill de data vieja **con guardia SEPOMEX** (el breadcrumb a veces da el municipio "San Pedro", degradaría "San Pedrito"). Nunca tocar `perito_correccion`/`usuario_correccion`. Base: `scratchpad/backfill_colonia_breadcrumb.py`. Ver memoria. |
...
| 136 | ⏳ | **Motor: usar edad_efectiva/grado_remodelacion de comps** — conectar la edad efectiva ponderada (remodelación) al motor. RIESGOSO: validar con validador; ojo doble conteo (calcEdadEfectiva fija + factorConserv ya suben remodeladas). |
| 137 | ⏳ | **Flywheel Capa 1** — diccionario aprendido coto→colonia/edad desde correcciones del perito; el enricher lo consulta para auto-rellenar lo nuevo. CP del anuncio/mapa (reverse-geocode) como ancla objetiva. |
