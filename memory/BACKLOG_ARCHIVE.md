...
**Bloque 1 — Créditos prepago (#192).** El usuario pidió el checkout de $380/flip que había quedado pendiente, pero a mitad de la conversación cambió de idea: en vez de un cobro suelto, quiso el sistema completo de créditos prepago (paquetes comprados por adelantado, usables para Flipping y/o OPI) — la idea que se había dejado como backlog futuro en la sesión anterior. Se construyó: `creditos_ledger` (ya existía en `User`) ganó campo `uso` por entrada; `core/creditos.py` ganó `gastar_credito()` y `saldo_efectivo(uso=)`; `CreditPurchase` nuevo + router `creditos_compra.py` con 4 paquetes (flip_1 solo-flipping, chico/mediano/grande mixtos); flujo completo comprar→comprobante→confirmación admin→crédito aplicado, con `CreditosCheckoutModal.jsx` reusable y panel `/admin/creditos` nuevo. Bug real encontrado y arreglado en vivo: `otorgar_credito` guardaba `uso="mixto"` literal en vez de traducir a `"cualquiera"`, dejando los paquetes mixtos inservibles para OPI. Verificado backend completo por API, y luego (a pedido explícito del usuario, "pruébalo") una segunda pasada 100% en vivo: compra real en `/flipping`, comprobante subido, admin con login real (cuenta de prueba temporal, borrada al final) confirmando desde el panel, saldo reflejado en el dashboard del investor.

**Bloque 2 — Datos reales (SMTP + banco).** El usuario dio la cuenta bancaria real (Bitso: banco Nvio, CLABE, beneficiario) y una cuenta Gmail (`propvalu.contacto@gmail.com`) para SMTP. El primer intento con contraseña de aplicación de Gmail no funcionó (Google bloquea App Passwords en cuentas nuevas) — se pivoteó a pedirle el App Password igual y esta vez sí llegó; ambos correos del flujo de créditos (aviso a admin, confirmación a cliente) se probaron en vivo y llegaron.

**Bloque 3 — Dominio propio (#193-194).** El usuario compró `propvalu.com.mx` y pidió conectarlo. Se usó el MCP de Vercel (list_projects/get_project) y Railway (generate-domain) para identificar los proyectos y generar el dominio custom del backend. El usuario pidió reiteradamente "métete tú" para hacerlo directo en su navegador — el Browser pane propio no tenía sesión en Vercel/Cloudflare (son cuentas del usuario), así que se conectó la extensión "Claude in Chrome" al Chrome real del usuario (una vez que la instaló/conectó) para operar con su sesión ya logueada. Se agregaron 3 registros CNAME en Cloudflare (root+www→Vercel, api→Railway). El primer deploy después de conectar el dominio falló: **Root Directory del proyecto Vercel estaba en `./`**, instalando un `package.json` viejo de un backend Node/Express abandonado (con puppeteer) que vivía en la raíz del repo — causando `react-scripts: command not found`. Se corrigió a `frontend` tras confirmar que el repo de GitHub usa como raíz lo que localmente es `Pagina-Valuacion-con-Ai--main/` (un nivel distinto al de la carpeta local `valuation-ai/`, fuente de confusión). Redeploy manual disparado vía el botón "Redeploy" de Vercel (no había forma de forzarlo solo por API), verificado `READY` y `www.propvalu.com.mx` respondiendo 200 en vivo. De ahí, el usuario preguntó si el dominio traía correo — no, así que se armó **Cloudflare Email Routing**: MX/SPF/DKIM agregados automático, destino `propvalu.contacto@gmail.com` (auto-verificado, es la cuenta dueña), y 5 reglas de reenvío (`ventas@`, `soporte@`, `notificaciones@`, `admin@`, `contacto@`) para que el usuario pueda filtrarlas en carpetas de Gmail después (los filtros de Gmail en sí quedaron pendientes, sin tiempo/tokens).

**Aparte:** se generó también el ícono oficial de PropValu (Building2 del header, extraído directo del DOM del sitio en vivo para exactitud pixel-perfect, no una recreación a mano) como SVG standalone y como badge con fondo — guardado en `frontend/public/` y documentado en memoria para no tener que re-extraerlo en el futuro.
...
## 12 Ago 2026 — Scraper revivido + mojibake/CASAS_Y_TERRENOS + caché motor Sheet→Mongo reactivado + Brave

El scheduler mensual llevaba ~22h caído sin ningún rastro en logs (descartados reboot, crash de Windows y fallo de auth Mongo, los tres verificados en vivo). Relanzado + montado un watchdog independiente (Bash + `Monitor` persistente, chequeo cada 4 min) que reinicia el proceso si muere y solo declara éxito real cuando el propio log dice "Todas las tareas completadas" — el criterio ingenuo inicial (contar `estado≠completada` en `progress.json`) resultó falso, porque el scheduler solo reintenta tareas con más de 30 días de antigüedad, no en cada corrida.

**2 bugs del scraper arreglados** (ya anotados como pendientes desde una sesión previa, #155): (1) mojibake de zona "Tonalá"/"Tlajomulco de Zúñiga" era **triple**-codificado vía `cp1252`, no el simple latin-1 que `_normalizar()` intentaba corregir — fix con hasta 3 rondas iterativas de `cp1252` contra las zonas canónicas. (2) `CASAS_Y_TERRENOS`: MeiliSearch a veces devuelve `hit["slugs"]` como string en vez de dict → `'str' object has no attribute 'get'`, arreglado con guardia `isinstance`. Además, **pausa entre tareas calibrada por portal** (antes 12-22 min fijo): PINCALI/CASAS_Y_TERRENOS/MITULA bajan a 2-5 min (sin antibot real, mismo criterio que ya tenía el enricher desde junio), PROPIEDADES_COM a 5-10 min, INMUEBLES24/VIVANUNCIOS se quedan conservadores.

**Hallazgo grande:** el caché del motor (`cache_consolidado.json`/`cache_index.json`) llevaba 5+ semanas sirviendo una foto congelada de Google Sheets — el scraper dejó de escribirle desde el 08-jul (retirado, límite 10M celdas) pero el builder (`actualizar_cache_consolidado.js`) seguía leyendo de ahí, así que toda la data scrapeada desde entonces nunca llegó al motor. El intento de migrar a Mongo de junio se había revertido por "regresión fuerte"; un segundo intento con dedup `es_duplicado_secundario` (27-jun) se revirtió **sin medirse**. Re-probado hoy con 118,115 docs actuales de Mongo: `actualizar_cache_desde_mongo.py` → 24,676 comps limpios (cobertura de año 44.6%). Validador 62 OPIs Sheet-vs-Mongo dio **empate técnico** (±10% +1.7pp, ±15/±20% -1.6/-3.3pp, error absoluto mejor) — ya no es la regresión fuerte de junio (12 vs 10 fuera de ±20%, antes ~21 vs 11). Decisión del usuario: quedarse con Mongo en prod.

El sesgo residual (la mediana cambió de signo, +9.2%→-9.2%) se aisló casi 100% al pool `lote_grande_cus`/`suma_partes` (18/86 casos); el pool `exacta` (53/86, el grueso) quedó casi neutro — no es un problema de la migración en sí. Revisar pm2T de terreno colonia por colonia no dio una causa única limpia; se cortó ahí a propósito para no caer en "cazar OPIs atípicos". Causa real identificada: `pm2t_semilla.json` (calibración de BACKLOG #120, generada 29-jun) nunca se refrescó contra el pool nuevo de Mongo — agendado como #159, requiere su propia validación offline, no se tocó hoy.

Investigado también por qué se agotó Tavily (1,000/mes, confirmado en vivo con HTTP 432): el log de comps encontrados mostró ráfagas de cientos de búsquedas en un solo día (495 el 02-ago, 87 colonias distintas) — no es tráfico orgánico, son corridas del validador con `--n` grande. Agregado Brave Search como 3er escalón de `buscarWeb()` (falta la API key real del usuario). Todo commiteado y pusheado (`dfe2d7a` + 2 auto-backups diarios pendientes). BACKLOG #155 cerrado parcial, #156/#159/#160/#161 actualizados/nuevos.

## 24 Ago 2026 (noche) — Ross-Heidecke investigado a fondo con avalúo real + pm2t_semilla ponderado + enricher apagado

Sesión aparte de la federación (más temprano el mismo día): se retomó una duda sobre si la depreciación por edad Ross-Heidecke "es muy dura" en el motor. Revisando `opi_perito.xlsx` (Excel de perito) la tabla RH resultó estar huérfana ahí (ninguna fórmula la lee) — pero el usuario aportó un **avalúo SHF real** (folio 26080015287, formato oficial Infonavit) que cambió la conclusión: el PDF documenta literalmente `Fed=1-0.5*(x+x²)` y el número reportado (`0.64` para edad=34/vida=70) coincide exacto con `getRH()` del motor. Confirmado además que **vida útil=70 es fija**, no condicionada a calidad como asumía la regla vieja — hasta un inmueble Interés Social del avalúo real usó vida=70.

**Hallazgo más importante:** el perito real NO blendea físico+mercado — el "Valor Concluido" es 100% el comparativo, el físico se reporta aparte como cross-check. `server.py::calculate_valuation` sí hace `estimated_value = comparativo×0.80 + físico×0.20` siempre — esa arquitectura de blend fijo es la pieza mal diseñada, no la curva de edad. Se corrigió `_depreciacion_lab()` a vida=70 fija (commit `97e8620`), tocando solo `result_lab_rh` (campo paralelo en observación desde el 4-ago, no afecta `estimated_value` en prod).

Validado en 3 niveles (tablas completas en `MOTOR_ANTECEDENTES.md`): 614 OPIs de `cerebro_datos.json` (RH70 sale igual/peor que la fórmula vieja, con y sin el clamp ±30% — confirma que el blend es el problema, no la curva), 1 caso real aislado (RH70 fue la que más se acercó al físico real), y 22 avalúos reales de PropValu en staging (cambiar a RH70 mueve <1% en promedio — el blend+clamp amortiguan casi todo). Confirmado también que el motor JS (`motor_remi_api.js`) NO usa RH en el flujo normal de comparables — solo en `sumaDePartes()` (fallback raro); el flujo normal ya deprecia con su propio `factorEdad` calibrado desde julio, sin tocar hoy.

**`pm2t_semilla.json` refrescado y ponderado (BACKLOG #159, cerrado):** regenerado desde `cerebro_datos.json` actual (515→562 colonias) + `build_pm2t_semilla.py` ahora pondera cada comp de terreno por antigüedad de la OPI (peso 1.0 primeros 3 meses, decae por trimestre, usando la fecha codificada en el folio). Validador: agregado casi neutro pero cero regresión y mejora consistente en todos los `lote_grande_cus` que cambiaron. Backup guardado.

Aparte: el enricher del scraper entró en loop de crash (los 3 shards casi simultáneo, ~200s cada uno) — apagado a propósito, pendiente diagnosticar la causa antes de relanzar. También se encontró y arregló localmente un conflicto de versión `starlette`/`fastapi` que tenía el backend local caído desde el inicio de la sesión (paquete MCP compartiendo el mismo Python global subió `starlette` sin querer). Todo commiteado y pusheado (`97e8620` + `04c5a6d`).
