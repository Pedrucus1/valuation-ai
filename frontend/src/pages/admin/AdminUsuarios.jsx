...
import {
  Search, ChevronDown, MoreVertical, ShieldCheck,
  Ban, CheckCircle2, Eye, ChevronLeft, ChevronRight, X, Users,
} from "lucide-react";
...
const ESTADO_COLORS = {
  ...
  kyc_pendiente: "bg-amber-100 text-amber-700",
};
  ...
  return (
    <AdminLayout badges={badges}>
      <div className="max-w-6xl mx-auto space-y-5">
        ...
        <PageHeader icon={Users} title="Gestión de Usuarios"
          subtitle={cargando ? "Cargando…" : `${total || usuarios.length} usuarios registrados · ${filtrados.length} en vista actual`} />
        ...
        <AdminCard icon={Users} title={`Usuarios (${filtrados.length})`}>
          <div className="overflow-x-auto">
            <table className="w-full">
              <GradThead cols={["Usuario", "Tipo", "Plan", "Estado", "Val.", "Registro", ""]} />
              <tbody className="divide-y divide-slate-50">
                {paginados.length === 0 ? (
                  ...
                  paginados.map((u) => (
                    <tr key={u.id} className="hover:bg-[#F0FAF5]/50 transition-colors">
                      <td className="px-5 py-3">
                        <p className="text-sm font-semibold text-[#1B4332] leading-snug">{u.nombre}</p>
                        <p className="text-xs text-slate-400">{u.email}</p>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${TIPO_COLORS[u.tipo] || "bg-slate-100 text-slate-500"}`}>
                          {u.tipo}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 relative">
                        ...
                        {menuAbierto === u.id && (
                          <div className="absolute right-4 top-10 z-20 bg-white border border-[#B7E4C7] rounded-xl shadow-lg py-1 w-44">
                            <button onClick={() => { setModalUsuario(u); setMenuAbierto(null); }}
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="bg-gradient-to-r from-[#1B4332] to-[#2D6A4F] px-5 py-4 flex items-start justify-between">
              <div>
                <h2 className="font-['Outfit'] font-bold text-white text-lg">{modalUsuario.nombre}</h2>
                <p className="text-[#D9ED92]/70 text-sm">{modalUsuario.email}</p>
              </div>
              <button onClick={() => setModalUsuario(null)} className="text-white/50 hover:text-white mt-0.5">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-5">
              ...
              <div className="flex gap-2">
                {modalUsuario.estado !== "suspendido" ? (
                  <button onClick={() => { cambiarEstado(modalUsuario.id, "suspendido"); setModalUsuario(null); }}
                  ...
                  <button onClick={() => { cambiarEstado(modalUsuario.id, "activo"); setModalUsuario(null); }}
