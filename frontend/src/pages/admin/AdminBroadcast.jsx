...
const PLANTILLAS = [
  ...
  },
  {
    ...
    cuerpo: "Hola {{nombre}},\n\nYa puedes capturar y subir fotos del inmueble directamente desde la cámara de tu teléfono al llenar el formulario de valuación.\n\nNo más pasos innecesarios — haz click en 'Tomar foto' en el paso 2 del formulario.\n\nSigue mejorando con PropValu,\nEquipo PropValu",
  },
];
  ...
  return (
    <AdminLayout>
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div>
          <h1 className="font-['Outfit'] text-2xl font-bold text-[#1B4332]">Broadcast Email</h1>
          <p className="text-slate-400 text-sm mt-0.5">Envía mensajes segmentados a valuadores e inmobiliarias</p>
        </div>
        ...
        {resultado && (
          <div className="flex items-start gap-3 bg-green-50 border border-green-200 rounded-2xl p-4">
            ...
            <button onClick={() => setResultado(null)} className="ml-auto text-green-400 hover:text-green-600">✕</button>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              <label className="block text-sm font-semibold text-[#1B4332] mb-3">
                Segmento de destinatarios <span className="text-red-500">*</span>
              </label>
              ...
              {seg && (
                <div className="mt-2 flex items-center gap-2 text-sm text-[#1B4332] bg-[#D9ED92]/30 rounded-xl px-4 py-2.5">
            ...
            {/* Plantilla */}
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              <label className="block text-sm font-semibold text-[#1B4332] mb-3">Plantilla (opcional)</label>
              <div className="grid grid-cols-1 gap-2">
                {PLANTILLAS.map((p) => (
                  <button key={p.id} type="button" onClick={() => cargarPlantilla(p.id)}
                    className={`text-left px-4 py-3 rounded-xl border text-sm transition-all ${plantillaSeleccionada === p.id ? "border-[#52B788] bg-[#52B788]/10 text-[#1B4332] font-semibold" : "border-slate-200 text-slate-600 hover:border-slate-300"}`}>
                    {p.nombre}
                  </button>
                ))}
              </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-4">
              ...
              {preview && asunto && (
                <div className="border border-slate-200 rounded-xl overflow-hidden">
                  <div className="bg-slate-50 px-4 py-2 text-xs text-slate-400 font-semibold border-b border-slate-100 uppercase tracking-wide">
                    Vista previa — como lo recibiría el destinatario
                  </div>
                  <div className="p-4">
              ...
              {!segmento && asunto && (
                <div className="flex items-center gap-2 text-yellow-700 text-xs bg-yellow-50 border border-yellow-100 rounded-xl px-4 py-3">
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              <h2 className="font-semibold text-[#1B4332] text-sm mb-3">Historial reciente</h2>
              {HISTORIAL.length === 0 ? (
