    ...
    <div className="bg-[#1B4332] rounded-2xl p-5 w-full max-w-sm mx-auto shadow-xl">
      ...
      {media_url && (
        <div className="rounded-xl overflow-hidden mb-3">
          <img src={media_url} alt="" className="w-full h-32 object-cover"
          ...
          {slot === "download" ? (
            <div className="flex flex-col items-center gap-2">
              <div className="w-40 h-9 rounded-xl bg-slate-200 flex items-center justify-center">
                <span className="text-xs text-slate-400">Descarga en {SLOTS.find(s=>s.id===slot)?.countdown}s…</span>
              </div>
            </div>
          ) : (
...
// ── Tab: Creatividades ─────────────────────────────────────────────────────────
const TabCreatividades = ({ ads, isLoading, onEdit, onDelete, onToggle, onPreview, onNew, slotLabel }) => (
            ...
            <div key={ad.id} className="px-5 py-4 flex items-start gap-4 hover:bg-slate-50 transition-colors">
              ...
              <div className="flex flex-col gap-1.5 shrink-0">
                <button onClick={() => onPreview(ad)}
                  className="p-1.5 rounded-lg text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-colors" title="Previsualizar">
                  <Monitor className="w-4 h-4" />
                </button>
                <button onClick={() => onEdit(ad)}
                  className="p-1.5 rounded-lg text-slate-400 hover:text-[#1B4332] hover:bg-[#D9ED92]/30 transition-colors" title="Editar">
                ...
                <button onClick={() => onDelete(ad.id)}
                  className="p-1.5 rounded-lg text-slate-300 hover:text-red-500 hover:bg-red-50 transition-colors" title="Eliminar">
...
// ── Tab: Campañas ──────────────────────────────────────────────────────────────
const TabCampanias = ({ ads, isLoading, onEdit, slotLabel }) => {
  // Group ads by advertiser name (or "Sin campaña")
  ...
  return (
    <div className="flex flex-col gap-4">
      {Object.entries(grouped).map(([name, items]) => {
        ...
        const totalImps = items.reduce((s, a) => s + (a.impressions || 0), 0);
        // Date range
        const starts = items.flatMap(a => a.starts_at ? [new Date(a.starts_at)] : []);
        const ends = items.flatMap(a => a.ends_at ? [new Date(a.ends_at)] : []);
          ...
          <Card key={name} className="border-0 shadow-sm">
            <CardContent className="p-0">
              ...
              <div className="px-5 py-4 border-b border-slate-100 flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-['Outfit'] font-bold text-slate-800 text-sm">{name}</p>
                  <div className="flex flex-wrap gap-3 mt-1 text-xs text-slate-500">
                    <span className="flex items-center gap-1">
                      <ImageIcon className="w-3.5 h-3.5" /> {items.length} creatividad{items.length !== 1 ? "es" : ""}
                    </span>
                    {minStart && (
                      <span className="flex items-center gap-1">
                        <Calendar className="w-3.5 h-3.5" />
                        {minStart.toLocaleDateString("es-MX")}
                        {maxEnd ? ` → ${maxEnd.toLocaleDateString("es-MX")}` : " → continua"}
                      </span>
                    )}
                    {slotsUsed.length > 0 && (
                      <span className="flex items-center gap-1">
                        <Clock className="w-3.5 h-3.5" />
                        {slotsUsed.map(s => slotLabel(s)).join(", ")}
                      </span>
                    )}
                    {items[0]?.geo && (
                      <span className="flex items-center gap-1">
                        <MapPin className="w-3.5 h-3.5" /> {items[0].geo}
                      </span>
                    )}
                  </div>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-xl font-bold text-[#1B4332]">{totalImps.toLocaleString("es-MX")}</p>
                  <p className="text-xs text-slate-400">impresiones</p>
                </div>
              </div>
              ...
              <div className="px-5 py-3 flex items-center gap-3">
                {active > 0 && (
                  <span className="flex items-center gap-1.5 bg-[#f0faf4] border border-[#b7e4c7] text-[#1B4332] text-xs px-3 py-1 rounded-full font-semibold">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#52B788]" /> {active} activo{active !== 1 ? "s" : ""}
                  </span>
                )}
                {paused > 0 && (
                  <span className="flex items-center gap-1.5 bg-amber-50 border border-amber-200 text-amber-700 text-xs px-3 py-1 rounded-full font-semibold">
                    <span className="w-1.5 h-1.5 rounded-full bg-amber-400" /> {paused} pausado{paused !== 1 ? "s" : ""}
                  </span>
                )}
              </div>

              {/* Creatividades in this campaign */}
                  ...
                  <div key={ad.id} className="px-5 py-3 flex items-center gap-3 hover:bg-slate-50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-1 mt-0.5">
                        {(ad.slots || []).map(s => (
                          <span key={s} className="text-xs text-slate-400">{slotLabel(s)}</span>
                        ))}
                      </div>
                    </div>
                    <p className="text-sm font-semibold text-slate-600 shrink-0">
                      {(ad.impressions || 0).toLocaleString("es-MX")} imp.
                    </p>
                    <button onClick={() => onEdit(ad)}
                      className="p-1.5 rounded-lg text-slate-300 hover:text-[#1B4332] hover:bg-[#D9ED92]/30 transition-colors">
...
const TabMetricas = ({ ads, isLoading }) => {
  ...
  const pausedAds = ads.filter(a => !a.active);

  // Impressions by slot
  const impsBySlot = SLOTS.map(slot => {
    const total = ads.filter(a => (a.slots || []).includes(slot.id))
      .reduce((s, a) => s + (a.impressions || 0), 0);
    return { ...slot, total };
  });
  const maxSlotImps = Math.max(...impsBySlot.map(s => s.total), 1);

  // Top 3 ads by impressions
  const topAds = [...ads].sort((a, b) => (b.impressions || 0) - (a.impressions || 0)).slice(0, 5);

  // Est. cost (at $5 per imp for simplicity)
  const estCost = totalImps * 5;
    ...
    <Card className="border-0 shadow-sm">
      <CardContent className="p-0">
        <div className="flex flex-col items-center justify-center py-16 text-slate-400">
          ...
          <p className="text-xs mt-1">Las métricas aparecen una vez que tus anuncios empiecen a mostrarse.</p>
        </div>
      </CardContent>
    </Card>
  );
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Impresiones totales", value: totalImps.toLocaleString("es-MX"), icon: BarChart2, color: "text-[#1B4332]" },
          { label: "Creatividades activas", value: activeAds.length, icon: Eye, color: "text-[#52B788]" },
          { label: "Creatividades pausadas", value: pausedAds.length, icon: ImageIcon, color: "text-amber-500" },
          { label: "Costo estimado", value: `$${estCost.toLocaleString("es-MX")}`, icon: TrendingUp, color: "text-blue-600" },
        ].map(kpi => (
      ...
      {/* Impresiones por slot */}
      <Card className="border-0 shadow-sm">
        <CardContent className="p-5">
          <p className="text-sm font-bold text-slate-700 mb-4">Impresiones por slot de tiempo</p>
          <div className="flex flex-col gap-3">
            {impsBySlot.map(slot => (
              <div key={slot.id}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-medium text-slate-600">{slot.label}</span>
                  <span className="text-xs font-bold text-[#1B4332]">{slot.total.toLocaleString("es-MX")}</span>
                </div>
                <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-[#52B788] rounded-full transition-all duration-700"
                    style={{ width: `${(slot.total / maxSlotImps) * 100}%` }}
                  />
                </div>
              </div>
            ))}
      ...
      <Card className="border-0 shadow-sm">
        <CardContent className="p-0">
          <div className="px-5 py-4 border-b border-slate-100">
            <p className="text-sm font-bold text-slate-700">Top creatividades por impresiones</p>
          </div>
          <div className="divide-y divide-slate-100">
            {topAds.map((ad, i) => (
              <div key={ad.id} className="px-5 py-3 flex items-center gap-3">
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
                  i === 0 ? "bg-[#D9ED92] text-[#1B4332]" : "bg-slate-100 text-slate-400"
                }`}>{i + 1}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-800 truncate">{ad.title}</p>
                  {ad.advertiser && <p className="text-xs text-slate-400">{ad.advertiser}</p>}
                </div>
                <div className="text-right shrink-0">
                  <p className="text-sm font-bold text-[#1B4332]">{(ad.impressions || 0).toLocaleString("es-MX")}</p>
                  <p className="text-xs text-slate-400">imp.</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <Card className="border-0 shadow-sm bg-gradient-to-br from-[#1B4332] to-[#2D6A4F] text-white">
        <CardContent className="p-6">
          ...
          <p className="text-xs text-white/50 text-center">
            Para activar campañas y gestionar facturación contacta a{" "}
...
const AnunciantesConsolaPage = () => {
  ...
  const [activeTab, setActiveTab] = useState("creatividades");
    ...
    try {
      ...
      const r = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          geo: form.geo || null, media_url: form.media_url || null,
  ...
  const handleEdit = (ad) => {
    ...
    setForm({
      tag: ad.tag || "", title: ad.title || "", body: ad.body || "", slots,
      media_url: ad.media_url || "", advertiser: ad.advertiser || "", geo: ad.geo || "",
  ...
  const handlePreviewAd = (ad) => {
    setForm({
      tag: ad.tag || "", title: ad.title || "", body: ad.body || "", slots: ad.slots || [],
      media_url: ad.media_url || "", advertiser: ad.advertiser || "", geo: ad.geo || "",
      starts_at: "", ends_at: "",
    });
  ...
  const handleNew = () => {
    setShowForm(true); setEditingId(null); setForm(EMPTY_FORM); setPreviewSlot(null);
    setActiveTab("creatividades");
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA]">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 flex flex-col gap-6">
        ...
        {/* ── Formulario + Previsualización (solo en tab Creatividades) ── */}
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <Field label="Descripción">
                        <textarea value={form.body} onChange={e => set("body", e.target.value)}
                          placeholder="Ubicación premium. Amenidades completas." rows={3}
                  ...
                  {form.slots.length > 0 && (
                    <div className="flex gap-1.5 flex-wrap">
                      {form.slots.map(s => {
                        ...
                        return (
                          <button key={s} onClick={() => setPreviewSlot(s)}
        ...
        {/* ── Tab content ── */}
        {activeTab === "creatividades" && (
          <TabCreatividades
            ads={ads} isLoading={isLoading}
            onEdit={handleEdit} onDelete={handleDelete}
            onToggle={handleToggleActive} onPreview={handlePreviewAd}
            onNew={handleNew} slotLabel={slotLabel}
          />
        )}
        {activeTab === "campanias" && (
          <TabCampanias ads={ads} isLoading={isLoading} onEdit={handleEdit} slotLabel={slotLabel} />
        )}
        {activeTab === "metricas" && (
          <TabMetricas ads={ads} isLoading={isLoading} />
        )}
      </div>
    </div>
  );
