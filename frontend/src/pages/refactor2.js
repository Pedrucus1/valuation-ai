...
const regexRender = /\{\/\* Tab: Resumen \*\/\}[\s\S]*\{\/\* Tab: Data Exchange \*\/\}[\s\S]*?\{activeTab === "data_exchange" && <DataExchangeTab \/>\}/;
...
if (regexRender.test(content)) {
  content = content.replace(regexRender, renderBlockNew);
  fs.writeFileSync(path, content, 'utf8');
  console.log('Render block successfully replaced!');
} else {
