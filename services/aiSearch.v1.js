...
2. FILTRO DE NIVEL SOCIOECONÓMICO (CRÍTICO): La colonia sujeto es "${propertyData.neighborhood}". Incluye propiedades de colonias cercanas SOLO si tienen un nivel socioeconómico y rango de precios SIMILAR a la sujeto. DESCARTA comparables cuyo snippet indique claramente un segmento distinto: colonias de lujo premium cuando la sujeto es clase media, o colonias populares/periféricas cuando la sujeto es residencial. Por ejemplo, si la sujeto es clase media-alta, excluye fraccionamientos de lujo con precios 2-3x superiores al resto de resultados. Si la colonia o precio no es claro, inclúyela.
3. Un anuncio individual DEBE tener en el snippet un precio específico en pesos como número (ej. $3,500,000 o 3500000). Si el precio NO aparece explícitamente en el snippet, devuelve price: null. NUNCA inventes ni estimes precios — si no está en el texto, es null. Si price es null el comparable será descartado automáticamente.
4. Si un dato no está en el snippet, devuelve null — NUNCA adivines. EXCEPCIÓN IMPORTANTE: para tipos Departamento, Oficina o Local comercial, si el snippet menciona m² de construcción pero NO m² de terreno, pon en "land_area" el mismo valor que "construction_area" (en estos inmuebles el terreno proporcional equivale al área construida del anuncio).
5. Excluye remates bancarios, adjudicaciones o ventas forzosas.
6. ${propertyData.land_regime !== 'EJIDAL' ? 'Excluye propiedades ejidales.' : 'Puedes incluir ejidales.'}
7. Usa la URL real del anuncio tal como aparece arriba.
8. Devuelve máximo ${count} comparables. Si hay menos anuncios individuales útiles, devuelve los que haya.
9. El valuador homologará diferencias de tamaño — incluye aunque sea más grande o pequeño.
...
[
  {
    ...
    for (const c of (parsed || [])) {
        ...
        if (typeof c.price_per_sqm === 'string') {
            c.price_per_sqm = parseFloat(c.price_per_sqm.replace(/[$,\s]/g, '')) || null;
        }
        if (typeof c.construction_area === 'string') c.construction_area = parseFloat(c.construction_area) || null;
        if (typeof c.land_area === 'string') c.land_area = parseFloat(c.land_area) || null;
    }

    // Filtrar comparables sin precio válido
    const conPrecio = parsed.filter(c => c.price && Number(c.price) > 0);
    ...
    if (withPsqm.length >= 4) {
        const sorted = [...withPsqm].sort((a, b) => a.price_per_sqm - b.price_per_sqm);
        const mid = Math.floor(sorted.length / 2);
        const median = sorted.length % 2 !== 0
            ? sorted[mid].price_per_sqm
            : (sorted[mid - 1].price_per_sqm + sorted[mid].price_per_sqm) / 2;
        const lo = median * 0.4;
        const hi = median * 2.5;
        const inRange = final.filter(c => !c.price_per_sqm || c.price_per_sqm === 0 || (c.price_per_sqm >= lo && c.price_per_sqm <= hi));
        if (inRange.length >= 3) {
            const removed = final.length - inRange.length;
            if (removed > 0) console.log(`Filtro $/m²: ${removed} comparables fuera del rango [${Math.round(lo)}–${Math.round(hi)} $/m²] (mediana ${Math.round(median)}) descartados.`);
            filteredByLocation = inRange;
        }
    }
    ...
    const deduped = [];
    for (const c of (filteredByLocation.length > 0 ? filteredByLocation : conPrecio)) {
    ...
    if (deduped.length < 10) {
        ...
        if (fbItems.length > 0) {
            ...
            try {
                ...
                console.log(`Fallback 1 añadió ${fbValid.length} comparables → total: ${deduped.length}`);
            } catch (e) {
